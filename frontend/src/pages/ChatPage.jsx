...
export default function ChatPage() {
  ...
  const send = async (question) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: "assistant", content: res.answer, messageId: res.message_id }]);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-screen bg-base text-ink font-sans">
      <header className="flex items-center gap-2.5 px-6 py-4 border-b border-border bg-surface">
        <img src="/logo.webp" alt="Logo" className="w-8 h-8 rounded-full ring-1 ring-border" />
        ...
        <div className="max-w-[720px] mx-auto px-6 py-8 flex flex-col gap-6">
          {messages.length === 0 && (
            <div className="flex flex-col items-center text-center gap-4 py-20">
              <img src="/logo.webp" alt="" className="w-14 h-14 rounded-full shadow-card ring-1 ring-border" />
                    ...
                    <li key={t}>
                      <button
                        ...
                        className="rounded-full border border-border bg-surface px-3.5 py-1.5 text-[13px]
                          text-ink shadow-card hover:border-accent/50 hover:bg-elevated transition-colors"
                      >
                        {t}
                      </button>
                    </li>
      ...
      <div className="px-6 py-4">
        <form
          ...
          className="max-w-[720px] mx-auto flex items-end gap-2 rounded-2xl border border-border
            bg-surface shadow-card px-4 py-2.5 focus-within:border-accent/50 transition-colors"
        >
          <textarea
            ref={textareaRef}
            value={input}
        ...
        {error && <p className="max-w-[720px] mx-auto mt-2 text-[12px] text-danger">{error}</p>}
      </div>
    </div>
  );
}
